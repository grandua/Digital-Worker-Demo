...
This repo contains SciCalc, a scientific calculator built with .NET 10 MAUI Blazor Hybrid and xUnit (projects under `Domain/`, `Presentation/`, `UnitTests/` and `IntegrationTests/`). See [Presentation/SciCalc.App/README.md](Presentation/SciCalc.App/README.md) for the project layout, verification commands, MAUI workload caveat, platform targets, and behavior decisions.

## SciCalc solutions

- `SciCalc.sln` — `Domain/SciCalc.Domain` + `UnitTests/SciCalc.UnitTests` + `IntegrationTests/SciCalc.Packaging.IntegrationTests` only. Workload-free: root-level verification needs no MAUI workloads.
- `SciCalc.App.sln` — adds the MAUI `SciCalc.App` app project (`Presentation/SciCalc.App`). For machines with the `maui` workloads installed; building it elsewhere fails with `NETSDK1147`.
