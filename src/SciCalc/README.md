...
- `SciCalc.sln` (repo root) — `SciCalc.Domain` + `SciCalc.Tests` only. This is the workload-free verification solution; `dotnet test SciCalc.sln` never touches MAUI targets.
- `SciCalc.App.sln` (repo root) — `SciCalc.Domain` + `SciCalc` (this project) + `SciCalc.Tests`. For machines with MAUI workloads installed (IDE builds, packaging, `dotnet build SciCalc.App.sln`).
