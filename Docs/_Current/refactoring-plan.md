...
# Solution Structure Rules Compliance — Restructure [****] (CURRENT for card 6a9db31d, 2026-09-06)

**Task:** Review SciCalc and UrlShortener solution/project structures against the Solution Structure Rules and fix all non-minor violations. Behavior-preserving layout changes only.
**Scope:** SciCalc (2349 LOC → physical layer folders required above 1000 LOC), UrlShortener (362 LOC → in-project layer separation stays, per the <1000 LOC exception).
**Verification:** `dotnet build` 0 errors; `dotnet test` green (UrlShortener unit + integration tests; SciCalc.UnitTests + SciCalc.Packaging.IntegrationTests); SciCalc.App.sln build — MAUI app project fails with NETSDK1147 in the sandbox (no workloads), expected and accepted per repo docs.

## Findings to fix (all non-minor)

| ID | Violation | Fix |
|----|-----------|-----|
| F1 | SciCalc uses generic `src/`, `tests/` root folders instead of physical layer folders | Move projects under `Domain/`, `Presentation/`, `UnitTests/`, `IntegrationTests/` |
| F2 | `SciCalc.sln` virtual folders are `src`/`tests` | Recreate with `Domain`, `Presentation`, `UnitTests`, `IntegrationTests` |
| F3 | `SciCalc.App.sln` same as F2 | Same as F2 |
| F4 | MAUI presentation project named `SciCalc` (no role segment) | Rename project to `SciCalc.App` (csproj `SciCalc.App.csproj`; explicit `RootNamespace`/`AssemblyName` `SciCalc` kept so XAML `Class` directives and `ApplicationId` are unchanged) |
| F5 | `SciCalc.Tests` lacks plural `UnitTests` suffix | Rename to `SciCalc.UnitTests` (folder `UnitTests/SciCalc.UnitTests`, namespace `SciCalc.UnitTests`) |
| F6 | `SciCalc.Packaging.Tests` are integration-level tests under generic `tests/` | Move to `IntegrationTests/SciCalc.Packaging.IntegrationTests`, namespace renamed, hard-coded `src/SciCalc` path assertions updated |
| F7 | `UrlShortener.sln` has no solution virtual folders | Add `Presentation`, `UnitTests`, `IntegrationTests` mirroring physical layout |
| F8 | Web project named `UrlShortener` (no role segment) | Rename project + folder to `UrlShortener.Web` (`RootNamespace` `UrlShortener` kept; namespaces `UrlShortener.*` unchanged) |
| F9 | `UrlShortener.Tests` mixes unit and TestServer integration tests; generic name | Split into `UnitTests/UrlShortener.UnitTests` (ShortCodeTests, ShortLinkTests) and `IntegrationTests/UrlShortener.IntegrationTests` (LinkApiTests, RateLimitTests, ConcurrencyTests, TestServerFixture) |

Not changed (compliant / exception): UrlShortener in-project `Domain/`, `Data/`, `Api/` folders (<1000 LOC exception); `SciCalc.Domain` naming; product prefixes (consistent per solution).

## Steps (one logical move per step; references updated in the same step)

- [x] S1: Move `src/SciCalc.Domain` → `Domain/SciCalc.Domain`.
- [x] S2: Move `src/SciCalc` → `Presentation/SciCalc.App`; rename csproj to `SciCalc.App.csproj`; explicit `<AssemblyName>SciCalc</AssemblyName>` + `<RootNamespace>SciCalc</RootNamespace>`; ProjectReference → `..\..\Domain\SciCalc.Domain\SciCalc.Domain.csproj`.
- [x] S3: Move `tests/SciCalc.Tests` → `UnitTests/SciCalc.UnitTests`; rename csproj; namespace `SciCalc.UnitTests`; ProjectReference → `..\..\Domain\SciCalc.Domain\SciCalc.Domain.csproj`.
- [x] S4: Move `tests/SciCalc.Packaging.Tests` → `IntegrationTests/SciCalc.Packaging.IntegrationTests`; rename csproj; namespace → `SciCalc.Packaging.IntegrationTests`; update `RepoRoot`-based path assertions (`src/SciCalc` → `Presentation/SciCalc.App`).
- [x] S5: Recreate `SciCalc.sln` and `SciCalc.App.sln`: updated project paths/names; virtual solution folders `Domain`, `Presentation`, `UnitTests`, `IntegrationTests`.
- [x] S6: Move + rename `UrlShortener/` → `UrlShortener.Web/`, csproj → `UrlShortener.Web.csproj`, explicit `<RootNamespace>UrlShortener</RootNamespace>`.
- [x] S7: Split `UrlShortener.Tests` → `UnitTests/UrlShortener.UnitTests` + `IntegrationTests/UrlShortener.IntegrationTests` (namespaces renamed; ProjectReference → `..\..\UrlShortener.Web\UrlShortener.Web.csproj`).
- [x] S8: Recreate `UrlShortener.sln` with `Presentation`, `UnitTests`, `IntegrationTests` virtual folders.
- [x] S9: Update stale layout references in the SciCalc README (path-only edits). Verify builds and tests.
