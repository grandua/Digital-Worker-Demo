...
public sealed class PackagingManifestTests : ConformanceTests
{
    ...
    private XDocument Manifest => Repo.Xml("Presentation", "SciCalc.Maui", "Platforms", "Windows", "Package.appxmanifest");
    ...
    {
        Assert.NotEmpty(Repo.Dir("Presentation", "SciCalc.Maui", "Resources", folder).EnumerateFiles());
    }
    ...
    private void AssertUnderResources(string include)
    {
        ...
        Assert.True(source.Exists, $"'{include}' must resolve to an existing file under Presentation/SciCalc.Maui/Resources/.");
    }
    ...
    private FileInfo ResolveProjectAsset(string include)
    {
        ...
        string[] path = ["Presentation", "SciCalc.Maui", .. segments];
    ...
    private FileInfo ManifestAssetFile(string? value)
    {
        ...
        string[] segments = ["Presentation", "SciCalc.Maui", "Platforms", "Windows", .. relative.Split(Path.DirectorySeparatorChar)];
