...
- `Domain/SciCalc.Domain.UnitTests/` — xUnit v3 unit tests for the Domain layer, co-located with its subject under the Domain layer folder.
- `Presentation/SciCalc.Maui.UnitTests/` — xUnit v3 packaging-conformance unit tests for this project (csproj, `Package.appxmanifest`, resources), co-located beside it under the Presentation layer folder.
...
Always name the solution file explicitly — the repo also hosts `UrlShortener/UrlShortener.slnx` from an unrelated demo, so a bare `dotnet test` is ambiguous. Caveat: on SDK 10.0.302, do **not** pass `--nologo` — it routes `dotnet test` through the legacy VSTest path, which reports "Zero tests ran (error: 1)" even though all tests pass.
